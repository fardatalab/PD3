...
// This file is copied from /opt/mellanox/doca/applications/common/src/dpdk_utils.c.
// Changes that we apply to this file will be marked with a 'PD3' comment.
...
{
	// PD3:
	// We are using 9000B Ethernet jumbo frames, so we need to ensure the mbuf size
	// can accommodate the full packet + Ethernet header + headroom.
	// 9000 (MTU) + 14 (Ethernet) + 128 (RTE_PKTMBUF_HEADROOM) = 9142.
	// Round up to nearest 128B-aligned size → 9216 (128 * 72).
	//
	// *mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL", total_nb_mbufs, MBUF_CACHE_SIZE, 0,
	// 				    RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
	*mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL", total_nb_mbufs, MBUF_CACHE_SIZE, 0,
					    128 * 72, rte_socket_id());
	if (*mbuf_pool == NULL) {
 ...
 * @port [in]: the port ID
 * @app_config [in]: application dpdk config struct
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t
 ...
 */
static void
dpdk_ports_fini(struct application_dpdk_config *app_config, uint16_t nb_ports)
{
	int result;
	uint16_t port_id;
	uint16_t n;
	...
	for (port_id = 0, n = 0; port_id < nb_ports; port_id++) {
		...
		if (++n >= app_config->port_config.nb_ports)
			break;
	}
}
...
dpdk_fini()
{
	...
	if (result < 0) {
		DOCA_LOG_ERR("rte eal cleanup failed, error=%d", result);
		return;
	}

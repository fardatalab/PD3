...
bool RingBuffer::Produce(const char* data, size_t size) {
...
bool RingBuffer::Produce(const char* data, const char* data2, size_t size1, size_t size2) {
  auto producer_idx = producer_index_.value.load(std::memory_order_relaxed);
  auto consumer_idx = consumer_index_.value;
  size_t distance = 0;
...
// TODO: change
bool RingBuffer::Consume(char* data, size_t& size) {

...
doca_error_t
dpf_open_doca_device_with_pci(const char *pci_addr, jobs_check func, struct doca_dev **retval)
{
...
doca_error_t
dpf_host_init_core_objects(AppState* state)
{
...
doca_error_t
dpf_create_core_objects(AppState* state, uint32_t max_bufs)
{
	doca_error_t res;
	struct doca_pe *pe;
...
doca_error_t
dpf_start_context(AppState* state)
{
...
doca_error_t
dpf_init_core_objects(AppState* state, uint32_t workq_depth, uint32_t max_bufs)
{
	doca_error_t res = dpf_create_core_objects(state, max_bufs);
	if (res != DOCA_SUCCESS)
		return res;
	res = dpf_start_context(state);
	return res;
}

doca_error_t
dpf_destroy_core_objects(AppState* state)
{
...
void
dpf_host_destroy_core_objects(AppState* state)
{
	doca_error_t res;

	DOCA_LOG_INFO("Stopping host mmap");
	res = doca_mmap_stop(state->src_mmap);
	if (res != DOCA_SUCCESS)
			DOCA_LOG_ERR("Failed to stop mmap: %s", doca_error_get_name(res));
...
char *
dpf_hex_dump(const void *data, size_t size)
{
	/*

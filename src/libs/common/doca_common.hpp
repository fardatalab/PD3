 ...
 */
doca_error_t dpf_open_doca_device_with_pci(const char* pcie_addr, 
                                       jobs_check func,
 ...
 */
doca_error_t dpf_host_init_core_objects(AppState* state);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
doca_error_t dpf_init_core_objects(AppState* state, uint32_t workq_depth, uint32_t max_chunks);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
doca_error_t dpf_destroy_core_objects(AppState* state);
 ...
 */
void dpf_host_destroy_core_objects(AppState* state);
 ...
 */
char *dpf_hex_dump(const void *data, size_t size);

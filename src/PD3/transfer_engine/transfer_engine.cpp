...
TransferEngine::~TransferEngine()
{
  for (auto job : all_jobs_) {
    ...
    if (job->dma_task != nullptr) {
      doca_task_free(doca_dma_task_memcpy_as_task(job->dma_task));
      job->dma_task = nullptr;
    }
    delete job;
  }
  all_jobs_.clear();
...
bool TransferEngine::Initialize(const TransferEngineConfig& config)
{
  ...
  // need to create the refresh request batches
  // for (int i = 0; i < 1024; i++) {
  //   ClientRequestBatch* batch = new ClientRequestBatch;
  //   batch->bytes = 0;
  //   batch->capacity = config.workq_depth;
  //   batch->size = 0;
  //   batch->requests = new ClientRequestT[1024]; // TODO: add larger batches
  //   // refresh_req_batches_.push_back(batch);
  // }
  // refresh_req_batch_q_ = new ClientRequestBatchQueue(1024);
...
void TransferEngine::SetPrefetchRequestQueue(PrefetchRequestBatchQueue* prefetch_req_batch_q)
{
  prefetch_req_batch_q_ = prefetch_req_batch_q;
}
...
// ClientRequestBatchQueue* TransferEngine::GetRefreshRequestQueue()
// {
//   return refresh_req_batch_q_;
// }
...
void TransferEngine::RunWithFixedBatches(int num_batches)
{
  running_.store(true);
  std::thread t([this, num_batches]() {
    this->WorkerThreadFixedBatches(num_batches);
  });
  worker_ = std::move(t);
}
...
bool TransferEngine::InitializeDma() 
{
  ...
  CHECK_DOCA(dpf_open_doca_device_with_pci(config_.dpu_pcie_addr.c_str(), nullptr, &doca_state_.dev));
  ...
  CHECK_DOCA(dpf_init_core_objects(&doca_state_, config_.workq_depth, num_bufs_in_inventory));
  ...
  if (config_.enable_transfer) {
    ...
    host_resp_buffer_addr = host_req_buffer_addr + final_req_buf_size;
  }
  ...
  for (int i = 0; i < config_.num_slots; ++i) {
    LOG_INFOF("slot[{}].req = {}, slot[{}].resp = {}", i, (uint64_t)unified_slots_[i].req, i, (uint64_t)unified_slots_[i].resp);
  }
  ...
  for (int i = 0; i < config_.num_slots * 4; ++i) {
    ...
    auto result = doca_dma_task_memcpy_alloc_init(
      dma_ctx_,
      job_ptr->src_buf,
      job_ptr->dest_buf,
      { .ptr = static_cast<void*>(job_ptr)},
      &job_ptr->dma_task
    );
    ...
    // TODO: think about whether to create the task hre
...
void TransferEngine::WorkerThread()
{
  ...
  while (running_.load(std::memory_order_relaxed)) {
    ...
    while (prefetch_deque_.get_num_slots() > 0) {
      PrefetchRequestBatch** batch = prefetch_req_batch_q_->front();
      if (batch) {
        auto batch_ptr = *batch;
        prefetch_req_batch_q_->pop();
...
void TransferEngine::WorkerThreadFixedBatches(int num_batches)
{
  // Pin this thread to a CPU core (core 9 for example)
  int core_id = 9;
  cpu_set_t cpuset;
  ...
  while (running_.load(std::memory_order_relaxed)) {
    ...
    RecycleSlots();  // recycle slots
    if (total_prefetched_batches_ == num_batches) break;
    if (pause) {
      std::this_thread::yield();
    }
    pause = false;
  ...
  LOG_INFOF("Total Batches: {}, Total Requests: {}", total_prefetched_batches_, requests);
...
void TransferEngine::HandleDmaCompletion(DmaJob* dma_job) {
  ...
  case REQ_BUFFER_METADATA_READ: {
    ...
    // LOG_INFOF("read_size: {}, consumer_idx: {}, progress idx: {}, producer_idx: {}", read_size, consumer_idx, progress_idx, producer_idx);
    ...
    while (bytes_completed < read_size) {
      ...
      const size_t MAX_READ_SIZE = 12000; // TODO: maybe make this config driven
      ...
      if (slot_read_size > available_bytes) {
        ...
        *reinterpret_cast<uint32_t*>(unified_slot.req) = slot_read_size + sizeof(ProtocolHeader);
        ...
        *reinterpret_cast<uint32_t*>(unified_slot.req + sizeof(uint32_t) + sizeof(ProtocolHeader) + slot_read_size) = 1;
        ...
        // LOG_INFOF("slot_read_size: {}", slot_read_size);
        *reinterpret_cast<uint32_t*>(unified_slot.req) = slot_read_size + sizeof(ProtocolHeader);
        *reinterpret_cast<ProtocolHeader*>(unified_slot.req + sizeof(uint32_t)) = ProtocolHeader{RequestHeader::C2S_TRANSFER_REQUEST};
        auto read_offset = sizeof(uint32_t) + sizeof(ProtocolHeader);
  ...
  case REQ_BUFFER_DATA_READ: {
    ...
    // handle the refresh
    HandleWriteRefresh(unified_slot_id);
    // HandleRefresh(unified_slot_id); // TODO: what refresh needs to be called here?
  ...
  case RESP_BUFFER_METADATA_READ: {
    // TODO: the current code doesn't handle backpressure, left as a future task
...
void TransferEngine::PollRdmaCompletions()
{
  for (uint64_t i = 0; i < prefetch_deque_.get_num_full_slots(); ++i) {
    ...
    } else if (slot.status == SlotStatus::BLOCKED_DUE_TO_BACKPRESSURE) {
      // do we need to check whether there is a read outstanding?
      break;
    }
  }
  for (uint64_t i = 0; i < transfer_deque_.get_num_full_slots(); ++i) {
    ...
    if (slot.status == SlotStatus::PENDING_RDMA_OP) {
      ...
      if (io_ctx->recv_buffer->PollMessage()) {
        // LOG_INFOF("Completion received from remote memory backend at slot: {}", slot_idx);
        // we have a completion
        ...
        HandleReadRefresh(slot_idx);
      } else {
        break;
      }
    } else if (slot.status == SlotStatus::BLOCKED_DUE_TO_BACKPRESSURE) {
      // check if there is a read outstanding, or kick off a new write
      // TODO: later, not important for V1
    }
  }
}
...
void TransferEngine::RecycleSlots()
{
  ...
  if (!meta_read_outstanding_ && !transfer_deque_.full() && config_.enable_transfer) {
    meta_read_outstanding_ = true;
    ClientReqMetadataRead();
  }
...
bool TransferEngine::ClientReqMetadataRead()
{
  ...
  doca_dma_task_memcpy_set_src(dma_job->dma_task, src_doca_buf);
  doca_dma_task_memcpy_set_dst(dma_job->dma_task, dst_doca_buf);
                                         ...
                                         size_t size_2)
{
  ...
  // LOG_INFOF("Writing to client buffer: {} {} {} {} {} {}", slot_idx, slot_offset, data_offset, size, offset_2, size_2);
  ...
  // LOG_INFOF("src_address: {}, dst_address: {}, host_resp_addr_: {}", (uint64_t)src_address, (uint64_t)dst_address, (uint64_t)host_resp_addr_);
	result = doca_buf_inventory_buf_get_by_addr(doca_state_.buf_inv, 
																				  remote_mmap_ /*put mmap here*/,
																				  dst_address,
																				  TRANSFER_SIZE,
  ...
  // LOG_INFO("Submitted DMA write job to response buffer");
  return true;
}
...
bool TransferEngine::AgentDataWrite(uint32_t slot_idx)
{
  ...
  if (remaining_space_before_wrap < bytes_to_write) [[unlikely]] {
  ...
  if (TRANSFER_SIZE > remaining_space) {
    // we will need to do two writes
    // TODO
  }
...
bool TransferEngine::WriteToTransferResponseBuffer(uint32_t slot_idx)
{
  ...
  // handle wrap around
  if (resp_producer_idx_ < producer_idx) {
    LOG_INFOF("Handled wrap around: {} {} {}", producer_idx, resp_producer_idx_, data_size);
    // we have wrapped around
    auto remaining_space = PD3_RING_BUFFER_SIZE - producer_idx;
    ClientRespDataWrite(slot_idx, slot_offset, producer_idx, remaining_space);
    if (resp_producer_idx_ != 0) {
      ClientRespDataWrite(slot_idx, slot_offset + remaining_space, 0, resp_producer_idx_);
  ...
  ClientRespMetadataWrite(); // should this be done after the transfer is complete?
  // auto producer_idx = agent_meta_->producer_idx.value;
  // auto producer_pos = dpf::buffer::AgentBuffer::GetBufferIdx(producer_idx);
  // auto consumer_idx = agent_meta_->consumer_idx.value;
  // auto remaining_space = PD3_RING_BUFFER_SIZE - producer_pos;
  // if (slot.num_writes_remaining > remaining_space) {
  //   // we will need to do two writes
  //   // TODO
  // }

  // // TODO: write to the transfer response buffer
  return true;
}
...
bool TransferEngine::AgentDataRead()
{
  ...
  // max size since DMA won't work for larger sizes
...
uint64_t ParseRemoteClientWrite(char* req)
{
  // TODO: move this to a client specific implementation
  // TODO: add support to return multiple keys
  // hashmap impl, first 8 bytes are the key, next 8 are the value
  return *reinterpret_cast<uint64_t*>(req);
}

uint64_t ParseRemoteClientRead(char* req)
{
  // TODO: move this to a client specific implementation
  // TODO: add support to return multiple keys
  // hashmap impl, first 8 bytes are the key, next 8 are the value
...
// write refresh, triggered after the DMA read of write requests
void TransferEngine::HandleWriteRefresh(uint32_t slot_idx)
{
  auto& slot = unified_slots_[slot_idx];
  refresher_->SetScopedMode(false); // this is a write batch
  auto msg_size = *reinterpret_cast<uint32_t*>(slot.req) + sizeof(uint32_t);
  uint64_t offset = sizeof(uint32_t) + sizeof(ProtocolHeader);
  while (offset < msg_size) {
    // LOG_INFOF("offset: {}, msg_size: {}", offset, msg_size);
    auto req_size = *reinterpret_cast<size_t*>(slot.req + offset);
    auto curr_request = reinterpret_cast<TransferRequest*>(slot.req + offset + sizeof(size_t));
    if (!curr_request->is_read) {
      auto reqbuf = reinterpret_cast<char*>(curr_request + 1);
      refresher_->PassRefreshRequest(ParseRemoteClientWrite(reqbuf));
    }
    offset += req_size;
  }
  refresher_->FlushRefreshRequest();
}

// read refresh, triggered before DMA write of read requests
void TransferEngine::HandleReadRefresh(uint32_t slot_idx)
{
  // LOG_INFO("in handle read refresh");
  auto& slot = unified_slots_[slot_idx];
  refresher_->SetScopedMode(true); // this is a read batch
  auto msg_size = *reinterpret_cast<uint32_t*>(slot.resp) + sizeof(uint32_t);
  uint64_t offset = sizeof(uint32_t);
  while (offset < msg_size) {
    // LOG_INFOF("offset: {}, msg_size: {}", offset, msg_size);
    auto curr_request = reinterpret_cast<TransferResponse*>(slot.resp + offset);
    if (curr_request->is_read) {
      // send the read data through to the host view for bookkeeping
      auto reqbuf = reinterpret_cast<char*>(curr_request + 1);
      refresher_->PassRefreshRequest(ParseRemoteClientRead(reqbuf));
    }
    offset += curr_request->aligned_bytes;
  }
  refresher_->FlushRefreshRequest();
  // LOG_INFO("done with handle read refresh");
}

...
bool TransferClient::Initialize(const TransferClientConfig& config)
{
  ...
  // initialize the request and response buffers
	...
	auto resp_buffer_start = req_buffer_start + final_req_buf_size;
	response_buffer_ = new (resp_buffer_start) dpf::buffer::ResponseBuffer();
	LOG_INFOF("response_src: {}", (uint64_t)resp_buffer_start);

	// TODO: make this cleaner
	completion_buffer_ = new char[8192 * 32];
	completion_buffer_size_ = 8192 * 32;

	raw_memory_ = src_buffer;
...
{
	CompletionOutput output{nullptr, nullptr};
	if (last_read_size_ <= completion_buffer_pos_ || refresh_needed_) {
		auto producer_idx = response_buffer_->producer_index();
		// std::cout << "producer_idx: " << producer_idx << " consumer_idx_: " << consumer_idx_ << '\n';
		if (producer_idx == consumer_idx_) {
			return output;
		}
		size_t data_to_read = 0;
		if (producer_idx < consumer_idx_) {
			// LOG_INFOF("wrapped around: {} {}", producer_idx, consumer_idx_);
			...
			data_to_read = std::min(data_to_read, completion_buffer_size_);
			// LOG_INFOF("data to read: {}", data_to_read);
			std::memcpy(completion_buffer_, response_buffer_->buffer() + consumer_idx_, data_to_read);
			// std::memset(response_buffer_->buffer(), 0, data_to_read);
			// check whether this is 0
			auto t_resp = reinterpret_cast<TransferResponse*>(completion_buffer_);
			if (t_resp->aligned_bytes == 0) {
				// LOG_INFO("t_resp->aligned_bytes is 0");
				refresh_needed_ = true;
				return output;
			}
			// LOG_INFOF("t_resp->aligned_bytes: {}", t_resp->aligned_bytes);
			completion_buffer_pos_ = 0;
			last_read_size_ = data_to_read;
		}
	...
	if (response->aligned_bytes == 0) {
		// LOG_INFOF("t_resp->aligned_bytes is 0");
		// std::cout << "response->aligned_bytes is 0\n";
		refresh_needed_ = true;
		return output;
	}
	...
	consumer_idx_ = consumer_idx_ % PD3_RING_BUFFER_SIZE;
	return output;
}

void TransferClient::PrintStats()
{
	auto producer_idx = response_buffer_->producer_index();
	LOG_INFOF("producer_idx: {}, consumer_idx_: {}, refresh_needed_: {}", producer_idx, consumer_idx_, refresh_needed_);
}

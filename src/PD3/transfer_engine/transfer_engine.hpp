...
struct TransferEngineConfig {
  ...
  std::string local_addr; // local address or device to bind to
...
public:
  ...
  void SetPrefetchRequestQueue(PrefetchRequestBatchQueue* prefetch_req_batch_q);
  void SetRefresher(dpf::HostViewRefresher* refresher);
  
  /* if to be run on a dedicated thread */
  void Run();
  void RunWithFixedBatches(int num_batches);
  void RunNoDma();
  void Stop();
...
private:
  ...
  void HandleRefresh(uint32_t slot_idx);
  ...
  void WorkerThreadFixedBatches(int num_batches);
...
private:
  ...
  PrefetchRequestBatchQueue* prefetch_req_batch_q_;
  // Host view refresher
  dpf::HostViewRefresher* refresher_ = nullptr;
  // ClientRequestBatchQueue* refresh_req_batch_q_;
  // std::vector<ClientRequestBatch*> refresh_req_batches_;
  // uint64_t refresh_req_batch_idx_ = 0;

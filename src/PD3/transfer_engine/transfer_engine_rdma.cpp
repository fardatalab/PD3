...
bool TransferEngine::InitializeRdma()
{
  ...
  // bind to the local device
  if (!config_.local_addr.empty()) {
    struct sockaddr_in local_addr = {};
    std::memset(&local_addr, 0, sizeof(local_addr));
    local_addr.sin_family = AF_INET;
    inet_pton(AF_INET, config_.local_addr.c_str(), &local_addr.sin_addr);
    ret = rdma_bind_addr(conn_id_, (struct sockaddr*)&local_addr);
    if (ret) {
      LOG_ERROR("failed to bind to local device");
      return false;
    }
    LOG_INFO("Local device bound");
  }

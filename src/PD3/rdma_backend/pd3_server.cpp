...
static uint32_t align_to_eight_bytes(uint32_t pos) {
  return (pos + 7) & ~7;
}


PD3MemoryBackend::PD3MemoryBackend() {}
...
void PD3MemoryBackend::Configure(const JSON& config) {
  try {
    ...
    if (config.count("max_keys")) {
      max_keys_ = config["max_keys"].get<uint64_t>();
#if LOOKUP_TABLE == LOOKUP_TABLE_FLAT
      lookup_table_.resize(max_keys_);
#endif
    }
...
void PD3MemoryBackend::Bootstrap(uint64_t num_keys, std::function<uint64_t(uint64_t)> address_gen)
{
#if LOOKUP_TABLE == LOOKUP_TABLE_FLAT
  lookup_table_.resize(num_keys);
#endif
  for (uint64_t i = 0; i < num_keys; ++i) {
    lookup_table_[i] = address_gen(i);
  }
}

void PD3MemoryBackend::Bootstrap(const std::vector<uint64_t>& keys, const std::vector<uint64_t>& addresses)
{
#if LOOKUP_TABLE == LOOKUP_TABLE_FLAT
  lookup_table_ = keys;
#endif
  if (keys.size() != addresses.size()) {
    throw std::runtime_error("keys and addresses lengths must match");
  }
  for (uint64_t i = 0; i < keys.size(); ++i) {
    lookup_table_[keys[i]] = addresses[i];
  }
}

void PD3MemoryBackend::Run() {
  ...
  while (running_.load(std::memory_order_relaxed)) {
    ...
    LOG_INFO("Done with connection steps");
  }

}

void PD3MemoryBackend::RunAync() {
  running_ = true;
...
void PD3MemoryBackend::HandleClientDataPlane(RdmaDataSession* session) {
...
int PD3MemoryBackend::SetupConnectionChannel() {
...
int PD3MemoryBackend::RdmaListen() {
...
int PD3MemoryBackend::SetupClientResources(struct rdma_cm_id* id) {
...
int PD3MemoryBackend::CheckConnectionEvent(rdma_cm_event_type expected_event) {
...
int PD3MemoryBackend::ListenAndDispatchConnectionEvents(struct rdma_cm_id*& id, rdma_cm_event_type expected_event)
{
  struct rdma_cm_event* cm_event = nullptr;
  int ret = -1;
  while (running_.load(std::memory_order_relaxed)) {
...
void PD3MemoryBackend::HandleSlot(RdmaDataSession* session, uint32_t slot_id_to_process)
{
  // process the request
  auto request_buffer = reinterpret_cast<char*>(session->recv_buffers[slot_id_to_process]->buffer());
  auto msg_size = *reinterpret_cast<uint32_t*>(request_buffer) + sizeof(uint32_t);
  ...
  offset += sizeof(ProtocolHeader);
  if (header->header == RequestHeader::C2S_TRANSFER_REQUEST) {
    ...
    while (offset < msg_size) {
      ...
      // TODO: check if remaining size is less than the request size object
      auto req_size = *reinterpret_cast<size_t*>(request_buffer + offset);
      // offset += sizeof(size_t);
      // TODO: check if remaining size is less than the request size, if so then add to buffer
      auto curr_request = reinterpret_cast<offload::TransferRequest*>(request_buffer + offset + sizeof(size_t));
      // LOG_INFOF("offset: {}, msg_size: {}, curr_request->is_read: {}, curr_request->bytes: {}, curr_request->address: {}", offset, msg_size, curr_request->is_read, curr_request->bytes, curr_request->address);
      // LOG_INFOF("Total Requests Handled: {}", TOTAL_REQUESTS);
      //offset += sizeof(offload::TransferRequest);
      if (curr_request->is_read) {
        offload::TransferResponse* resp = reinterpret_cast<offload::TransferResponse*>(reply_buffer + reply_offset);
        reply_offset += offload::kTransferResponseSize;
        resp->bytes = curr_request->bytes;
        resp->req_id = curr_request->req_id;
        resp->is_read = curr_request->is_read;
        uint32_t segment = curr_request->address / MEMORY_REGION_SIZE;
        size_t r_offset = curr_request->address % MEMORY_REGION_SIZE;
        auto req_addr = (char*)memory_regions_[segment]->base_address + r_offset;
        std::memcpy(reply_buffer + reply_offset, (const void*)req_addr, curr_request->bytes);
        auto total_aligned_size = align_to_eight_bytes(curr_request->bytes + offload::kTransferResponseSize);
        reply_offset += (total_aligned_size - offload::kTransferResponseSize);
        resp->aligned_bytes = total_aligned_size;
      } else {
        offload::TransferResponse* resp = reinterpret_cast<offload::TransferResponse*>(reply_buffer + reply_offset);
        resp->is_read = curr_request->is_read;
        resp->bytes = offload::kTransferResponseSize;
        resp->aligned_bytes = align_to_eight_bytes(resp->bytes);
        resp->req_id = curr_request->req_id;
        reply_offset += resp->aligned_bytes;
        // write to local from the request buffer
        uint32_t segment = curr_request->address / MEMORY_REGION_SIZE;
        size_t r_offset = curr_request->address % MEMORY_REGION_SIZE;
        auto req_addr = (char*)memory_regions_[segment]->base_address + r_offset;
        std::memcpy((void*)req_addr, request_buffer + offset + sizeof(offload::TransferRequest) + sizeof(size_t), curr_request->bytes);
        // TODO: populate the address in the lookup_table and get the key via a User-Defined-Function
      }
      offset += req_size;
    }
    ...
    *(reinterpret_cast<uint32_t*>(reply_buffer + reply_offset)) = 1;
    ...
    session->send_slots--;
    // LOG_INFOF("Sent message with length: {} and total_bytes: {} from handleSlot", reply_offset, total_bytes_to_send);

  } else if (header->header == RequestHeader::C2S_PREFETCH_REQUEST) {
    
    // LOG_INFOF("Processing slot {}", slot_id_to_process - 8);
    // prepare the reply buffer
    auto reply_buffer = reinterpret_cast<char*>(session->reply_buffers[slot_id_to_process]);
    uint64_t reply_offset = sizeof(uint32_t);
    
    // we iterate to msg_size + sizeof(uint32_t) since the offset starts off with value = sizeof(uint32_t)
    size_t cnt = 0;
    size_t max_offset = msg_size;
    while (offset < max_offset) { 
      auto actual_size = 32;
      auto curr_request = reinterpret_cast<offload::PrefetchRequest*>(request_buffer + offset);
      offset += sizeof(offload::PrefetchRequest);
      offload::PrefetchResponse* prefetch_resp = reinterpret_cast<offload::PrefetchResponse*>(reply_buffer + reply_offset);
      // std::cout << curr_request->key << '\n';
      prefetch_resp->key = curr_request->key;
      prefetch_resp->consumer_state = dpf::buffer::CONSUMER_STATE_WRITTEN;
      prefetch_resp->size = align_to_eight_bytes(offload::kPrefetchResponseSize + VALUE_SIZE);
      auto address = lookup_table_[curr_request->key];
      auto segment = address / MEMORY_REGION_SIZE;
      auto physical_addr_offset = address % MEMORY_REGION_SIZE;
      char* physical_address = (char*)memory_regions_[segment]->base_address + physical_addr_offset;
      std::memcpy(reply_buffer + reply_offset + offload::kPrefetchResponseSize, physical_address, VALUE_SIZE);
      // mark end of response
      *(reinterpret_cast<uint64_t*>(reply_buffer + reply_offset + offload::kPrefetchResponseSize)) = 1;
      reply_offset += actual_size;
    }
    // send the reply
    auto total_bytes_to_send = reply_offset - sizeof(uint32_t);
    ...
    *(reinterpret_cast<uint32_t*>(reply_buffer + reply_offset)) = 1;
    ...
    // LOG_INFOF("Processed {} slot", TOTAL_REQUESTS);
    // TOTAL_REQUESTS++;
    
    // LOG_INFOF("Processed {} slot", slot_id_to_process - 8);
    // batches_processed++;
...
bool PD3MemoryBackend::PostReceive(RdmaDataSession* session,
                                        uint64_t request_id,
...
bool PD3MemoryBackend::PostSend(RdmaDataSession* session,
                                    uint64_t request_id,
                                    struct ibv_sge* sg_list,
                                    size_t num_sge,
                                    int flags) 
...
bool PD3MemoryBackend::PostWrite(RdmaDataSession* session,
                                    uint64_t request_id,
                                    struct ibv_sge* sg_list,
                                    size_t num_sge,
...
bool PD3MemoryBackend::WaitForCompletion(RdmaDataSession* session) {
  struct ibv_wc wc;
  while (true) {
    int ret = ibv_poll_cq(session->cq, 1, &wc);
    if (ret < 0) {

...
void PD3MemoryBackend::HandleSlot(RdmaDataSession* session, uint32_t slot_id_to_process)
{
  ...
  auto msg_size = *reinterpret_cast<uint32_t*>(request_buffer);
  ...
  if (header->header == RequestHeader::C2S_TRANSFER_REQUEST) {
    ...
    auto num_requests = msg_size / sizeof(offload::TransferRequest);
    for (int i = 0; i < num_requests; ++i) {
      auto curr_request = reinterpret_cast<offload::TransferRequest*>(request_buffer + offset);
      ...
      if (curr_request->is_read) {
        ...
        auto aligned_size = align_to_eight_bytes(curr_request->bytes);
        reply_offset += aligned_size;
        resp->aligned_bytes = aligned_size;
      } else {
        offload::TransferResponse* resp = reinterpret_cast<offload::TransferResponse*>(reply_buffer + reply_offset);
        reply_offset += offload::kTransferResponseSize;
        resp->is_read = curr_request->is_read;
        resp->aligned_bytes = 0;
        resp->bytes = 0;
        resp->req_id = curr_request->req_id;
    ...
    *(reinterpret_cast<uint32_t*>(reply_buffer + reply_offset)) = 1;
    ...
    size_t cnt = 0;
    size_t max_offset = msg_size + sizeof(uint32_t);
    while (offset < max_offset) { 
      ...
      prefetch_resp->size = 32;
      auto address = table_[curr_request->key];

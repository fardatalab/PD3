/**
* The PD3 server handles transfer requests and prefetch requests from the PD3 transfer engine
* This server requires some CPU to compute responses for requests, configurable but atleast 1 core
*/
#pragma once
...
#define LOOKUP_TABLE_FLAT 1
#define LOOOKUP_TABLE_MAP 2
#define LOOKUP_TABLE_BCHT 3

#define LOOKUP_TABLE LOOKUP_TABLE_FLAT

 
namespace dpf {

class PD3MemoryBackend {

  static constexpr size_t MEMORY_REGION_SIZE = 1_GiB;
  static constexpr size_t RDMA_BUFFER_SIZE = 8192 * 4;

  static constexpr size_t VALUE_SIZE = 8; // change this for different apps
...
public:
  ...
  void Bootstrap(uint64_t num_keys, std::function<uint64_t(uint64_t)> address_gen);
  void Bootstrap(const std::vector<uint64_t>& keys, const std::vector<uint64_t>& addresses);
...
private:
  ...
  void HandleSlot(RdmaDataSession* session, uint32_t slot_id_to_process);
...
private:
  ...
  std::vector<uint64_t> lookup_table_;
#elif LOOKUP_TABLE == LOOKUP_TABLE_MAP
  std::unordered_map<uint64_t, uint64_t> lookup_table_;
#endif 
};

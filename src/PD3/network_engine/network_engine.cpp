...
static void DebugIPv6(struct rte_mbuf* mbuf) {
  auto *eth = rte_pktmbuf_mtod(mbuf, rte_ether_hdr *);
  auto *ip6 = (rte_ipv6_hdr *)(eth + 1);

  if (ip6->proto != IPPROTO_TCP) return; // extension headers would fail here

  auto *tcp = (rte_tcp_hdr *)((char*)ip6 + sizeof(*ip6));
...
void ProcessPacket(struct rte_mbuf *mbuf, dpf::user_defined::HashmapParser& hashmap_parser) {
  ...
  uint16_t et = rte_be_to_cpu_16(eth_hdr->ether_type);

  if (et == RTE_ETHER_TYPE_IPV4) {
    struct rte_ipv4_hdr *ip_hdr = (struct rte_ipv4_hdr *)(eth_hdr + 1);
    if (ip_hdr->next_proto_id != IPPROTO_TCP) {
      // Not a TCP packet
      std::cout << "Received a non-TCP IPv4 packet\n";
      return;
    }
    ...
    // std::cout << "Received an IPv6 packet\n";
    struct rte_ipv6_hdr *ip6_hdr = (struct rte_ipv6_hdr *)(eth_hdr + 1);
    if (ip6_hdr->proto != IPPROTO_TCP) {
      // Not a TCP packet
      std::cout << "Received a non-TCP IPv6 packet\n";
      return;
    }
    int ip6_pkt_sz = rte_be_to_cpu_16(ip6_hdr->payload_len);
    int ip6_hdr_sz = sizeof(*ip6_hdr);

    struct rte_tcp_hdr *tcp_hdr = (struct rte_tcp_hdr *)((char *)ip6_hdr + ip6_hdr_sz);
    int tcp_hdr_sz = (tcp_hdr->data_off >> 4) * 4;
    int tcp_payload_sz = ip6_pkt_sz - tcp_hdr_sz;
    char* tcp_payload = (char *)tcp_hdr + tcp_hdr_sz;
    auto src_port = rte_be_to_cpu_16(tcp_hdr->src_port);
    auto dst_port = rte_be_to_cpu_16(tcp_hdr->dst_port);

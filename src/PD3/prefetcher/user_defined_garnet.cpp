...
const std::unordered_map<ClientRequestType, ClientRequestAction>
  request_action_map{
    { ClientRequestType::GET, ClientRequestAction::PREFETCH },
    { ClientRequestType::SET, ClientRequestAction::INVALIDATE },
    { ClientRequestType::DEL, ClientRequestAction::INVALIDATE },
    { ClientRequestType::RMW, ClientRequestAction::PREFETCH_INVALIDATE }
  };
...
ParseRemoteWrite(RemoteAddressT remote_addr, const char* local_addr, size_t len)
{
  ...
  std::vector<std::pair<RecordIdT, PrefetchRequest>> spilled{};

  size_t offset = 0;
  while (((len - offset) >= sizeof(RecordInfo)) &&
         get_record_info(local_addr + offset)->is_invalid()) {
  ...
  while ((len - offset) >= sizeof(RecordInfo)) {
    const char* record_ptr = local_addr + offset;
    ...
    spilled.emplace_back(RecordIdT{ key->payload, key->length },
                         PrefetchRequest{ .remote_addr = remote_addr + offset,
                                          .len = record_size });

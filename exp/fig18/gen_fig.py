        ...
        for threads in thread_counts:
            ...
            try:
                with open(filepath, 'r') as f:
                    lines = f.readlines()
                    # Parse lines like "Latency: p50: 120.75us" and "Latency: p99: 189.60us"
                    for line in lines[-2:]:  # Last two lines
                        line = line.strip()
                        if 'p50:' in line:
                            p50 = float(line.split('p50:')[1].replace('us', '').replace(',', '').strip())
                            p50_values.append(p50)
                        elif 'p99:' in line:
                            p99 = float(line.split('p99:')[1].replace('us', '').replace(',', '').strip())
...
workloads = ['SYNC_RDMA', 'Leap', 'REDY', 'PD3']

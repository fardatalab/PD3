...
- Run the client process: `./run.sh SYNC_RDMA`. This will generate result files under the `~/exp/fig18` directory

Clean Up:
- `Ctrl-C` the Garnet server on the compute node
- `Ctrl-C` the remote memory server on the memory node

Post finishing REDY_1, redo the above with `./run.sh REDY_2`, since Redy with Garnet is a bit unstable.
...
On the client node:
- Navigate to the experiment directory: `cd ~/exp/fig18`
- Run the client process: `./run.sh REDY_1`. This will generate result files under the `~/exp/fig18` directory
...
On the compute node:
- Run the Garnet application: `dotnet ~/garnet/pd3/publish/GarnetServer.dll --storage-tier --no-obj --memory 1g --segment 32m --bind 10.10.2.101 --no-pubsub --index 256m`

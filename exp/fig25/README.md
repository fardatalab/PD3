This figure compares the varying throughput of loading zone sizes
...
On the DPU:
- Run the PD3 DPU process: `cd ae/fig24_lz_<size> && sudo ./prefetcher_ne -- --json config.json`

On the client node:
- Run the client process: `~/garnet/fig24.sh `. This will print out the throughput of the Garnet application using only 128M of local memory, with the rest on remote memory
